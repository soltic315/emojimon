...
## 重要なルール（必ず遵守してください）
1. 著作権リスクになるような固有名詞（例：ポケモンと完全一致する名前）は使用しないこと
2. AIとのチャット完了後に、チャット内容をagent_history/<YYYYMMDDhhmm>.mdに保存し、コミットメッセージをログファイル名にして自動的にコミットすること
3. コードの修正後にREADME.md/GAME_DESIGN.md/AGENTS.mdなどのドキュメントに更新が必要ないか確認すること
s
## 📌 プロジェクト概要
