...
1. 変更対象と依存ファイルを確認する。
2. 最小差分で実装する。
3. `npm run lint` / `npm run typecheck` / `npm run test` を優先度順で確認する。
4. 仕様に影響する場合、`docs/GAME_DESIGN.md` を更新する。
5. 導入・運用に影響する場合、`README.md` を更新する。
6. AIとのチャット完了後、会話ログを必ず `docs/ai/YYYYMMDDhhmm.md` 形式で保存する。
7. ログの見出し・項目は `docs/ai/TEMPLATE.md` の形式に必ず準拠する。
8. ログ保存後は、作成したログファイル名（例: `docs/ai/202603010449.md`）でコミットする。
9. AIの作業はコミット完了までを必須とし、コミット未完了で終了しない。
...
- `docs/GAME_DESIGN.md`: ゲーム仕様の一次情報（ルール・数式・進行）
- `AGENTS.md`: AI/開発者向け作業規約
- `docs/ai/TEMPLATE.md`: AIチャットログの固定テンプレート
...
- [ ] 変更範囲が要求に一致している
- [ ] 型・Lint・テストに新規エラーがない
- [ ] 仕様変更が `docs/GAME_DESIGN.md` に反映されている
