...
- 手動セーブは `P` キーに加えてメニューの「セーブ」および設定画面の「セーブ」から実行できる
- 設定画面の「セーブデータ削除」は確認ダイアログで明示確認後に実行する
- セーブ対象:
  - プレイヤー状態
  - パーティ/ボックス
  - 所持品
  ...
  - 訪問済みマップ一覧
  - 日替わりチャレンジ状態
- 遷移時・バトル後にオートセーブ（セーブ中は画面右上に「💾 セーブ中…」トースト表示）
- 実績の解除状態をセーブデータに含む
...
- 30種の実績を4カテゴリに分類: BATTLE（バトル）/ COLLECTION（収集）/ EXPLORATION（探索）/ MASTERY（やりこみ）
...
| カテゴリ | 数 | 例 |
|---|---|---|
| BATTLE | 9 | 初バトル勝利、100勝、クリティカル撃破、全タイプ制覇 等 |
| COLLECTION | 9 | 初捕獲、図鑑10/30/全種、パーティ満員、全アイテム所持 等 |
| EXPLORATION | 6 | 全マップ踏破、1000歩、ショップ利用、進化達成 等 |
| MASTERY | 6 | Lv50/100達成、全技習得、所持金10万、全ジム制覇、全実績 等 |
...
- メニューのパーティ画面で `N` キーを押すと命名ダイアログを表示
- ニックネーム設定時は種族名の代わりにニックネームを表示し、種族名を小さく併記
