...
- タイトルの「はじめから」選択時は、フィールド表示の前にプロローグ演出を再生する
- プロローグ完了後に `WorldScene`（エモじタウン）へ遷移する
...
- 主要施設（回復所・ショップ）は、ワールドマップ上で視認しやすいように施設アイコンを重ね表示する
- 同じ施設座標はミニマップにも強調表示し、探索中に位置を把握しやすくする
- 本仕様は表示のみを対象とし、既存のドア遷移・会話・ショップ処理には影響しない
